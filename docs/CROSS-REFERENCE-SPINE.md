...
| `caused-by` | an RCA analyzes an incident | lessons ledger (`rca` → `incident`) |
| `mitigates` | a corrective action or lesson mitigates | lessons ledger |
| `origin` | an RCA/incident traces to its source ref | lessons ledger `origin` |
...
* a marker is malformed (an empty target list, or a target that is not one of
  the seven forms above — including `CA-NNNN` / `SUGGEST-NNNN`, which are valid
  ledger ids but not valid marker targets);
...
cmr-refs: ADR-0012, GR-11, RCA-0001
cmr-refs: LESSON-0001, INC-0001, #138
