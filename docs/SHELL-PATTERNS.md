...
| `SP-7` | a fetch piped straight into a shell (`curl … \| bash`) | it executes whatever the network answers with: no review, no pin, no digest, last-write-wins | 0 sites — infra is declared and flag-gated ON by default (AO-GR-6), never installed from a pipe |
