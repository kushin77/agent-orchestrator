...
| Hermes | missing | stubbed — `gateway/providers/hermes.py:22` real adapter code exists but flag-gated OFF by default (`gateway/providers/flags.py` `hermes_enabled()` fail-closed), catalog module `status: "retired"` (`gateway/catalog/modules/hermes/module.json:9`); standalone `integrations/hermes/client.py` is real, keyless, read-only HTTP but "never called at dispatch time" per its own docstring (ADR-0012) | wired (capability-space only) — `fleet/routing.py` + `routing.policy.json` `personas: [hermes, paperclip]`; **not reachable from `tiered.py`** |
