...
| Containerized sandboxing | PARTIAL | `guardrails/sandbox/` ships per-category security profiles (`restricted`/`standard`/`privileged`, fail-closed) with an injectable runtime; `docker.py` and `firecracker.py` are declared, and the `sandbox_runtime` flag defaults **on** (AO-GR-6); `guardrails/sandbox/enablement.py`'s `SandboxEnablement` is structurally OFF by construction as a separate safety property (refuses `enabled=True` at construction; only an explicit `enable()` flips it) | Enable the runtime path behind a flag, round-trip proven [#636](https://github.com/kushin77/agent-orchestrator/issues/636) |
