...
| C23 · reverse-direction codegen from a tagged diagram to real Terraform HCL (`blueprint-engine`), consumed PR-only and, if ever wired to a live apply path, gated per AO-GR-6/GR-5 (never un-gated, never applied outside the IaC/plan-review path) | futureproofing | kushin77/diagrams | https://github.com/kushin77/diagrams/issues/147 — completion signal: comment 5531345325, closing with evidence for `cloud_asset_api/blueprint_engine.py` and its API route | shipped | the generator produces HCL only from shapes tagged with a resource type and refuses untagged input, naming what it refused; a generator that infers a resource type fails | `gh api repos/kushin77/diagrams/issues/147/comments --jq 'any(.[]; .id == 5531345325)'` |
