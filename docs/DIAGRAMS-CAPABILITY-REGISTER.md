...
| C20 · an agent-facing read API for the module's own blueprint, usable without a browser session | decision-making, task-completion | kushin77/diagrams | https://github.com/kushin77/diagrams/issues/374 — completion signal: comment 5610698989, the parameterless single-repo form shipped with 19/19 tests at commits `71bf76e`/`8dfef0a` | shipped | a token-authenticated caller fetches a blueprint without a portal session, and the same call for a repository the route is not scoped to is refused rather than silently returning another repo's data | `gh api repos/kushin77/diagrams/issues/374/comments --jq 'any(.[]; .id == 5610698989)'` |
