...
| **Enabled by default** | rule 4 (same bundle) | `AO-GR-6` | `GR-28` | the 2026-09-21 reversal (`policy-gr5-enabled-by-default`); a separate rule id here, glued to IaC in `AGENTS.md`, and numbered `GR-28` in `scripts/check-terraform-iac.sh` (the hub's id for the *old* flag-gated-OFF mandate) |
...
| Tag authority | — | `AO-GR-28` | — | not a hub id; `AGENTS.md` cites it as `AO-GR-28` |
...
1. **`AGENTS.md` rule 4 and this file's `AO-GR-5`/`AO-GR-6` are one bundle and two
   ...
   `AGENTS.md` still presents them as one `GR-5`, and the change that fixed the
...
Naming the script-embedded `GR-28` here puts that id into the spine, so
`governance/controls/spine-coverage.yaml` carries a row for it (recorded
`not-applicable` — this repo deliberately does not enforce a rule it reversed).
