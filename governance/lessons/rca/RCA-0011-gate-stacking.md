# RCA-0011 — gates stacked on one box, with no lock and no queue

| Field | Value |
|---|---|
| RCA id | `RCA-0011` |
| Incident | `INC-0011` |
| Origin | `event: gate-stacking-2026-09-14` — the committed board snapshot does not carry #724 (see [`RCA-0014`](RCA-0014-stale-snapshot-no-trigger.md)) |
| Severity | `medium` |
...
- the pressure compounds with the shared tmpfs: one lane's 14.8 GB scratch log
  (`INC-0009`) starved the very gates that were stacking.
...
- `CA-0015` — a gate lock: one gate of record per worktree, a second invocation
...
`SUGGEST-0009` — **a gate is a resource claim, not just a script.** Anything that
...
- Ledger: `INC-0011`, `RCA-0011`, `CA-0015`, `SUGGEST-0009`.

## Follow-up

The incident closes with `CA-0015` (#724). Two related controls are separate
incidents and not merged into this one: the retry storm that multiplies the
stacked runs (`RCA-0012`) and the shared tmpfs that starves them (`RCA-0009`).
