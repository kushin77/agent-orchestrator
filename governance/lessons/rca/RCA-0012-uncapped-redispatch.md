# RCA-0012 — an uncapped retry stacked 137 gates and starved the box

| Field | Value |
|---|---|
| RCA id | `RCA-0012` |
| Incident | `INC-0012` |
| Origin | `event: uncapped-redispatch-2026-09-14` — the committed board snapshot does not carry #723 (see [`RCA-0014`](RCA-0014-stale-snapshot-no-trigger.md)) |
| Severity | `high` |
...
- `CA-0016` — a cap on redispatch in the runaway guard (`fleet/runaway.py`,
...
`SUGGEST-0010` — **a retry loop is a resource claim that must be bounded, and its
...
- `fleet/runaway.py` absent, and no `redispatch` / `max_attempts` / `retry_cap`
  ...
  2026-09-14).
- Ledger: `INC-0012`, `RCA-0012`, `CA-0016`, `SUGGEST-0010`.

## Follow-up

The incident closes with `CA-0016` (#723). The retry cap bounds the storm; it does
not serialize concurrent gates (`RCA-0011`) nor stop a single lane from filling
the shared tmpfs (`RCA-0009`) — each is its own control with its own issue.
