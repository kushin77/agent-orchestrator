...
def instructions(document: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        "4. After `make verify` is green and the PR is open, merge it with "
        "`bash scripts/merge-pr.sh --pr <number> --apply --delete-branch`, then close the issue. "
        "That entrypoint is the ONLY sanctioned merge path (#1145): it runs "
        "`scripts/check-squash-message.sh` first and REFUSES, by name, a PR whose composed squash "
        "message would land without its trailing ticket trailer. Never bypass it, and never invoke "
        "the merge subcommand of the GitHub CLI directly — a trailer-less squash message reds "
        "`make verify` on master for every lane, not just yours. "
