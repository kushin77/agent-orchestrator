...
def instructions(document: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        "`make verify`, and if it is green land it through the guarded merge entrypoint "
        "(`bash scripts/merge-pr.sh --pr <number>`) and close the issue. Only implement from "
        "scratch if no PR exists.\n"
        ...
        "4. After `make verify` is green and the PR is open, land it through the guarded merge "
        "entrypoint `bash scripts/merge-pr.sh --pr <number>` (dry run by default; "
        "`AO_MERGE_APPLY=1` executes). It runs `scripts/check-squash-message.sh` on the rendered "
        "squash message FIRST and refuses a trailer-less one by name "
        "(`squash-message-would-drop-trailer`) before any merge is attempted. Running the raw form "
        "`gh pr merge <number> --squash --delete-branch` instead of `bash scripts/merge-pr.sh "
        "--pr <number>` is FORBIDDEN: it consults no guard, and a body whose last paragraph is "
        "not the trailer block then lands a commit with no ticket reference, reddening "
        "`check-isolation-landed` on master for every lane (#1145). Then close the issue. NEVER "
        "leave a completed PR unmerged or the issue open.\n"
