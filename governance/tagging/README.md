...
| `e2e.py` | **The end-to-end chain.** One offline run proves a tag set derives a plan whose gates all resolve, whose matrix names the fired rules, whose mandate holds in this tree, and whose filing seam derives the same tag dimensions for a NEW issue — and then proves every futureproof classification mechanism (`class`/`pattern`/`template`/`rca`/`system`/`app`/`env-var`/`gov`/`issues`/`index`) has its authority declared, its gate wired into `make verify`, and its failure path exercised (19 links, each with a clean twin). |
