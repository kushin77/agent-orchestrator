...
#: Paths a *machine* rewrites inside a lane worktree, which therefore are not the
#: lane's uncommitted work (#834). Declared, and deliberately narrow — this is the
#: whole list, so a reviewer can see exactly what a reclaim is allowed to discard
#: without asking.
#:
#: Measured, not guessed. `fleet/tests/test_brain.py` drives the real brain loop,
#: whose `advance_epic_focus()` writes the checkout's own `.board/focus.json`
#: (committed `active_epic: 707` -> working tree `160`, sha `397bed81c9f6` ->
#: `171fbf96eaff`, 2026-09-15) — and `make verify` runs that suite. So a lane is
#: dirty in exactly this path the moment the close-out's own verification step
#: finishes, and step 8 then refused to reclaim the tree step 2 had just written
#: to: the close-out was racy against the fleet that owns the file, and against
#: itself. The file is board state owned by the fleet, its authoritative copy is
#: the main checkout's, and it is regenerated by the process that writes it.
#:
#: `.board/snapshot.json` is the same story: rewritten by the board refresher,
#: never lane-authored (checked `git log origin/master -- .board/snapshot.json`
#: — every touch is a governance/board-refresh commit, none lane feature work).
MACHINE_MANAGED_PATHS: tuple[str, ...] = (".board/focus.json", ".board/snapshot.json")

#: Path *prefixes* a machine owns wholesale inside a lane worktree. `.fleet/` is
#: gitignored runtime state on `master` (see repo `CLAUDE.md`/`AGENTS.md`); older
#: checkouts predate that `.gitignore` rule and so still show it as untracked,
#: which made the reaper KEEP a tree whose only "dirt" is regenerated scratch
#: state it never owned as lane work.
MACHINE_MANAGED_PREFIXES: tuple[str, ...] = (".fleet/",)


def _is_machine_managed(entry: str) -> bool:
    """Whether a dirty path is declared machine-managed, by exact path or prefix."""
    return entry in MACHINE_MANAGED_PATHS or any(
        entry.startswith(prefix) for prefix in MACHINE_MANAGED_PREFIXES
    )


def uncommitted_paths(path: Path | str) -> list[str]:
    """The paths carrying uncommitted work, including untracked files.

    Read with ``-z``: the default porcelain output *quotes* a path that needs it
    (spaces, a non-ASCII byte), and a quoted path can never match a declared set.
    A rename or copy record is followed by its source path, which is part of the
    same entry rather than a second dirty path.
    """
    result = git(path, "status", "--porcelain", "-z", "--untracked-files=all")
    ...
    while index < len(fields):
        ...
        if status[:1] in {"R", "C"} or status[1:2] in {"R", "C"}:
            index += 1  # the source path of a rename/copy is not a dirty path
        paths.append(entry[3:])
        index += 1
    return paths


def machine_managed_uncommitted(path: Path | str) -> list[str]:
    """The dirty paths that are declared machine-managed (:data:`MACHINE_MANAGED_PATHS`,
    :data:`MACHINE_MANAGED_PREFIXES`)."""
    return [entry for entry in uncommitted_paths(path) if _is_machine_managed(entry)]


def foreign_uncommitted(path: Path | str) -> list[str]:
    """The dirty paths that are the lane's *own* work — the ones a reclaim must refuse."""
    return [entry for entry in uncommitted_paths(path) if not _is_machine_managed(entry)]


def dirty(path: Path | str) -> bool:
    """Uncommitted work of any kind — including untracked files.

    Deliberately unfiltered: "is this tree dirty" is a different question from
    "may this tree be reclaimed" (which is :func:`close`'s, and narrows to
    :func:`foreign_uncommitted`).
    """
...
def close(identity: SessionIdentity, main: Path | str, force: bool = False) -> list[str]:
    """Remove the lane's worktree, refusing to discard *the lane's* work.

    Returns the list of reasons the lane was *kept*; empty means it was removed.
    The dirty test is narrowed to work the lane itself produced: a path a machine
    rewrites in the tree (declared in :data:`MACHINE_MANAGED_PATHS`) is not
    uncommitted work, and refusing on it made the reclaim racy against the fleet
    that owns `.board/focus.json` — and against the close-out's own gate, which
    dirties that file in the lane it is about to reclaim (#834). The refusal names
    the paths it is refusing on, so the operator does not have to guess which file
    stood in the way.
    """
    problems = []
    if identity.worktree.exists():
        foreign = foreign_uncommitted(identity.worktree)
        if foreign and not force:
            listed = ", ".join(foreign[:5])
            more = "" if len(foreign) <= 5 else f" (+{len(foreign) - 5} more)"
            return [f"worktree {identity.worktree} has uncommitted work ({listed}{more}); commit it or pass --force"]
        # `git worktree remove` applies its own dirty test and refuses a tree with
        # *any* modification or untracked file, so a reclaim that decided the dirt
        # is machine-managed has to say so to git as well — otherwise the decision
        # above changes nothing and the lane is kept anyway. (Measured: passing
        # `force=True` skipped only this module's check, so `--force` never reached
        # git and a forced reclaim still refused.)
        forced = ["--force"] if (force or machine_managed_uncommitted(identity.worktree)) else []
        remove = git(main_repo_root(main), "worktree", "remove", *forced, str(identity.worktree))
