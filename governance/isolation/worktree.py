...
MACHINE_MANAGED_PATHS: tuple[str, ...] = (".board/focus.json",)
...
def machine_managed_uncommitted(path: Path | str) -> list[str]:
    ...
    return [entry for entry in uncommitted_paths(path) if entry in MACHINE_MANAGED_PATHS]
...
def foreign_uncommitted(path: Path | str) -> list[str]:
    ...
    return [entry for entry in uncommitted_paths(path) if entry not in MACHINE_MANAGED_PATHS]
