...
def test_provision_creates_the_worktree_on_the_issue_branch(repo: Path, tmp_path: Path):
    identity = mint(263, "copilot-brain", "governance-isolation", worktree_root=tmp_path / "lanes")
    result = provision(identity, repo, base="HEAD")
...
def test_provision_is_idempotent_and_never_forks_a_second_worktree(lane, repo: Path):
    before = len(git(repo, "worktree", "list").stdout.strip().splitlines())
    again = provision(lane, repo, base="HEAD")
...
def test_two_lanes_hold_two_signatures_at_once(repo: Path, tmp_path: Path):
    ...
    provision(first, repo, base="HEAD")
    provision(second, repo, base="HEAD")
...
def test_a_lane_may_not_be_rooted_inside_the_shared_checkout(repo: Path):
    inside = mint(263, "copilot-brain", "governance-isolation", worktree_root=repo)
    with pytest.raises(ProvisionRefused):
        provision(inside, repo, base="HEAD")


def test_an_unresolvable_base_is_refused(repo: Path, tmp_path: Path):
    identity = mint(263, "copilot-brain", "governance-isolation", worktree_root=tmp_path / "lanes")
    with pytest.raises(ProvisionRefused):
        provision(identity, repo, base="origin/master")
...
def test_record_survives_a_reprovision(lane, repo: Path):
