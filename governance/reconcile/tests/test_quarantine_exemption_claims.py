...
The last test re-measures the **shipped** document's own rows with it. Its
failure message names the row and the reviewed edit it needs, because that is
exactly the signal rule 2 asks for and no other arm produces.
"""
...
def test_the_shipped_document_re_measures_its_own_rows():
    ...
    assert entries, "an empty document is measured by nothing (see real-tree-baseline.py)"
