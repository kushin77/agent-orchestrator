...
def parse_acceptance_commands(body: str) -> list[str]:
    ...
    a ```bash fenced block (every non-empty line is one command), and
    backtick-wrapped `` `cmd` `` spans. Commands are returned in source order.
    """
    ...
    for match in token.finditer(section):
        ...
        if span.startswith("```"):
            ...
            if command and not command.startswith("#") and not command.startswith("tier:"):
