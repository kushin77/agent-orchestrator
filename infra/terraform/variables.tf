...
variable "enable_erp_module" {
  description = "Serve the ERP module's portal surface (ERP-07, issue #652). In-module switch: surfaces.erp_module in portal/config/feature-flags.yaml. OFF by default (#1955): erp_module carries its own dedicated promotion gate (e2e/erp/golden_path.py, scripts/check-erp-portal.sh) requiring off-by-default, exempted from the GR-5 blanket enabled-by-default sweep."
  type        = bool
  default     = false # 1955-erp-own-promotion-gate
}
